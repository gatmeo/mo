\pitem[Olimpiada Rioplatense 2015 Level 3 P5]{%
    ...
    If there exists a number $a_i \le p-2$ then we have $n <n+p^i \le n+p^k<2n$ and
    $$n+p^i= \left( \overline{a_ka_{k-1} \cdots a_{i+1}(a_i+1)a_{i-1} \cdots a_1(p-1)} \right)_p.$$
    Therefore, according to Lucas theorem, we have 
    $$\binom{n+p^i}{n} \equiv \binom{a_k}{a_k} \cdots \binom{a_{i+1}}{a_{i+1}} \binom{a_i+1}{a_i} \cdots \binom{a_1}{a_1} \equiv a_i+1 \pmod{p}.$$
    Hence such exitence of $a_i \le p-2$ implies $p \nmid \binom{n+p^i}{n}$, a contradiction.
    ...
    https://artofproblemsolving.com/community/c6t45334f6h1265997_find_all_possible_values
}
